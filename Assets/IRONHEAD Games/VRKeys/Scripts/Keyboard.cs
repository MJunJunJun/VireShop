...
namespace VRKeys {
	...
	public class Keyboard : MonoBehaviour {
		...
		public void SetText (string txt) {
			...
			{
				textUsername = txt;
				OnUpdate.Invoke(textUsername);
			}
			if (password)
			{
				textPassword = txt;
				OnUpdate.Invoke(textPassword);
			}
		...
		public void AddCharacter (string character) {
			...
			if (username)
			{
				textUsername += character;
				OnUpdate.Invoke(textUsername);
			}
			if (password)
			{
				textPassword += character;
				OnUpdate.Invoke(textPassword);
			}
		...
		public void Backspace () {
			...
			if (username)
			{
				...
				{
					textUsername = textUsername.Substring(0, textUsername.Length - 1);
				}
				OnUpdate.Invoke(textUsername);
			}
			if (password)
		...
		public void Submit () {
			if (username)
			{
				OnSubmit.Invoke(textUsername);
			}
			if (password)
			{
				OnSubmit.Invoke(textPassword);
			}
		...
		private void UpdateDisplayText () {
			...
			if (username)
			{
				string display = (textUsername.Length > 37) ? textUsername.Substring(textUsername.Length - 37) : textUsername;
			...
			if (password)
			{
				string display = (textPassword.Length > 37) ? textPassword.Substring(textPassword.Length - 37) : textPassword;
